                ...
                element.steps.forEach(function(step) {
                    if (step.embeddings !== undefined) {
                        ...
                        step.embeddings.forEach(function(embedding) {
                            ...
                            var getScreenshotData = function (baseData) {
                                var screenshotData;
                                try {
                                    screenshotData = JSON.parse(Base64.decode(baseData.data));
                                    if (screenshotData.browserName && screenshotData.screenshot) {
                                        return {
                                            browserName: screenshotData.browserName,
                                            screenshot: 'data:image/png;base64,' + new Buffer(screenshotData.screenshot).toString('base64')
                                        };
                                    } else {
                                        throw new Error();
                                    }
                                } catch (e) {
                                    console.log('This is not a valid screenshot json object!');
                                    return {
                                        browserName: 'main',
                                        screenshot: 'data:image/png;base64,' + baseData.data
                                    };
                                }
                            };
                            ...
                            if (embedding.mime_type === 'text/plain') {
                                ...
                                var screenshotData = getScreenshotData(embedding);
                                name = name + '_' + screenshotData.browserName + '_' + Math.round(Math.random() * 10000) + '.png'; //randomize the file name
                                var filename = path.join(screenShotDirectory, name);
                                fs.writeFileSync(filename, screenshotData.screenshot, 'base64');
                                step.images.push('screenshot/' + name);
                            } else if (embedding.mime_type === 'image/png') {
                                step.images.push(getScreenshotData(embedding));
                            }
                        });
                    }
