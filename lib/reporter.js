...
var generateReport = function (options) {
    var featureDocument = jsonFile.readFileSync(options.jsonFile);
    var featureOutput = featureDocument;
    var latestVersion = options.cucumberVersion && options.cucumberVersion >= 6;
    if(latestVersion){
        featureOutput = featureDocument.gherkinDocuments;
    }
    ...
    var setStats = function (suite) {
        ...
        featureOutput.forEach(function (feature) {
            ...
            if(latestVersion){
                feature.elements = feature.feature.children;
            ...
            feature.elements.forEach(function (element) {
                ...
                if(latestVersion){
                    featureDocument.testCaseAttempts.forEach( function (testCaseObj){
                        if(testCaseObj.testCase.name === element.name){
                            element.testCaseObj = testCaseObj;
                            element.result = testCaseObj.testCase.result;
                        }
                    })

                    featureDocument.pickles.forEach( function (pickleObj){
                        if(pickleObj.name === element.name){
                            element.pickleObj = pickleObj;
                        }
                    })

                    element.steps.forEach( function (step) {
                        element.testCaseObj.testSteps.forEach( function (testStep){
                            if(testStep.text === step.text){
                                step.result = testStep.result;
                                if(testStep.result.exception){
                                    step.result.error_message = step.result.exception;
                                }
                                step.name = step.text;
                                step.match = {};
                                step.match.location = testStep.sourceLocation.uri;
                            }
                        })

                        element.pickleObj.steps.forEach( function (pickle){
                            if(pickle.text === step.text){
                                step.arguments = pickle.arguments;
                            }
                        })
                    })
                }
