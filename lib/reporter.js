...
var generateReport = function (options) {
     ...
     */
    var calculateDuration = function (durationInNanos) {
        var durationInMillis = options.timeInMilliseconds ? durationInNanos : _.floor(durationInNanos / 1000000);
    ...
    var setStats = function (suite) {
        ...
        featureOutput.forEach(function (feature) {
            ...
            //merge all background steps into their corresponding next scenarios
            if (options.mergeBackgroundSteps) {
                var pendingBackgroundSteps = [];
                feature.elements.forEach(function (element) {                
                    if (element.type === 'background') {
                        pendingBackgroundSteps = pendingBackgroundSteps.concat(element.steps);
                    } else {
                        element.steps = pendingBackgroundSteps.concat(element.steps);
                        pendingBackgroundSteps = [];
                    }                
                });
                feature.elements = _.filter(feature.elements, function(element) {
