...
var generateReport = function(options) {
    ...
    var setStats = function(suite) {
        ...
        var oneMinute = 60000000000;
        var oneSecond = 1000000000;
        ...
        //total suite/project runtime
        if (suite.totalTime / oneMinute >= 1) {
            //Converts the duration from nanoseconds to seconds and minutes (if any)
            suite.totalTime = Math.trunc(suite.totalTime / oneMinute) + "m " + _.round((suite.totalTime % oneMinute) / oneSecond) + "s";
        } else if (suite.totalTime / oneMinute < 1) {
            //display only seconds, if the test ran for less than a minute
           suite.totalTime = _.round(suite.totalTime / oneSecond) + "s";
        }
    ...
    var calculateDuration = function(duration) {
        ...
        if(!isNaN(duration)) {
            ...
            var time = Math.trunc(duration / oneMinute) + 'm ' + _.round((duration % oneMinute) / oneSecond) + 's';
